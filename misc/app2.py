...
def render_details_page(movies, movie_id):
    ...
    with col_info:
        ...
        if meta_bits:
            st.markdown(
                # f'<div class="cm-feature-year">{"  \u2022  ".join(meta_bits)}</div>',
                # unsafe_allow_html=True,
...
def render_home_page(movies, similarity):
    ...
    with st.container():
        st.markdown('<div class="cm-feature">', unsafe_allow_html=True)
        ...
        with col_info:
            ...
            st.markdown(
                f'<p class="cm-feature-overview">{selected_details["overview"][:320]}'
                f'{"\u2026" if len(selected_details["overview"]) > 320 else ""}</p>',
                unsafe_allow_html=True,
            )
            st.write("")
